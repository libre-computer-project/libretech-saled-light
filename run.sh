...
IO_BUF=$((((SALED_PANEL_WIDTH * SALED_PANEL_HEIGHT) * SALED_LED_PIXEL_BYTES + SALED_LED_FRAME_INIT_BYTES) * $SALED_PANEL_BUFFER_FRAMES))

./gen/"$SALED_GEN_ALGO" "$SALED_CONFIG"  | stdbuf -i$IO_BUF -o$IO_BUF ./led/$SALED_PANEL_LED "$SALED_CONFIG" > "$SPI_DEV"
